...
// Consulta para ranking de conductores
...
<head>
    ...
    <title>SGET - <?= $lang['mod_ranking_titulo'] ?? 'Ranking de Conductores' ?></title>
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white/5 dark:bg-white/[0.02] p-6 rounded-3xl border border-slate-200 dark:border-white/5 backdrop-blur-md">
                <div>
                    <div class="flex items-center gap-2.5">
                        <h1 class="text-2xl font-black text-slate-900 dark:text-white tracking-tight"><?= $lang['mod_ranking_titulo'] ?? 'Ranking de Conductores' ?></h1>
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-1"><?= $lang['sub_ranking_desc'] ?? 'Evaluación de desempeño, cantidad de viajes realizados y métricas del personal de conducción.' ?></p>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-[#121826] p-6 rounded-3xl border border-slate-200 dark:border-white/10 shadow-xl space-y-6">
                <div class="flex justify-between items-center">
                    <h2 class="text-base font-extrabold text-slate-900 dark:text-white flex items-center gap-2">
                        <i class="fas fa-trophy text-amber-400"></i> <?= $lang['tit_tabla_posiciones'] ?? 'Tabla de Posiciones y Rendimiento' ?>
                    </h2>
                </div>
                    ...
                    <table class="w-full text-left border-collapse text-xs">
                        <thead>
                            <tr class="border-b border-slate-100 dark:border-white/5 text-[10px] text-slate-400 uppercase font-bold">
                                <th class="pb-3 px-3"><?= $lang['lbl_posicion'] ?? 'Posición' ?></th>
                                <th class="pb-3 px-3"><?= $lang['conductor'] ?? 'Conductor' ?></th>
                                <th class="pb-3 px-3"><?= $lang['cfg_correo'] ?? 'Correo Electrónico' ?></th>
                                <th class="pb-3 px-3 text-center"><?= $lang['lbl_viajes_realizados'] ?? 'Viajes Realizados' ?></th>
                            ...
                            <?php if($resultado_ranking && $resultado_ranking->num_rows > 0): ?>
                                ...
                                <tr class="hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors">
                                    ...
                                    <td class="py-3.5 px-3 text-center">
                                        <span class="px-3 py-1 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 rounded-full text-[10px] font-extrabold uppercase"><?= $lang['lbl_activo'] ?? 'Activo' ?></span>
                                    </td>
                                </tr>
                                <?php $pos++; endwhile; ?>
                            <?php else: ?>
                                <tr>
                                    <td colspan="5" class="py-8 text-center text-slate-400 italic"><?= $lang['lbl_no_hay_conductores'] ?? 'No hay registros de conductores disponibles.' ?></td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
        ...
        <div class="bg-white dark:bg-[#121826] w-full max-w-md rounded-3xl p-6 border border-slate-200 dark:border-white/10 shadow-2xl space-y-4 transform scale-95 transition-all duration-300">
            <div class="flex justify-between items-center border-b border-slate-100 dark:border-white/5 pb-3">
                <h3 class="font-extrabold text-slate-900 dark:text-white text-base flex items-center gap-2">
                    <i class="fas fa-info-circle text-sky-400"></i> <?= $lang['tit_guia_ranking'] ?? 'Guía de Ranking de Conductores' ?>
                </h3>
            ...
            <ul class="space-y-2.5 text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
                <li class="flex items-start gap-2">
                    ...
                    <span><b><?= $lang['tit_tabla_posiciones'] ?? 'Tabla de Posiciones' ?>:</b> <?= $lang['item_guia_rank_1'] ?? 'El sistema genera automáticamente el orden de los conductores basándose en la cantidad de viajes registrados exitosamente en el despacho.' ?></span>
                </li>
                <li class="flex items-start gap-2">
                    <i class="fas fa-medal text-sky-400 mt-0.5"></i>
                    <span><b><?= $lang['item_guia_rank_2_tit'] ?? 'Top Rendimiento:' ?></b> <?= $lang['item_guia_rank_2_desc'] ?? 'Destaca visualmente a los tres (3) mejores conductores con iconos de medallas (Oro, Plata y Bronce) para facilitar la lectura del desempeño.' ?></span>
                </li>
            </ul>
