...
// Detección automática según parámetros clave si no viene especificado en el POST
if (empty($modulo) || in_array($modulo, ['crear', 'editar'])) {
    if (isset($_POST['id_via']) || isset($_POST['id_rut_via']) || isset($_POST['fec_via'])) {
        $modulo = 'viaje';
    } elseif (isset($_POST['restricciones']) || isset($_POST['id_usu_permisos'])) {
        $modulo = 'permisos';
    } elseif (isset($_POST['ori_rut']) || isset($_POST['nom_rut']) || isset($_POST['id_rut'])) {
        $modulo = 'ruta';
    } elseif (isset($_POST['num_doc_usu']) || isset($_POST['corre_usu']) || isset($_POST['nom_usu'])) {
        $modulo = 'usuario';
    } elseif (isset($_POST['pla_veh']) || isset($_POST['id_veh'])) {
        $modulo = 'vehiculo';
    }
}
...
switch ($modulo) {
    ...
    default:
        echo "<h3>Depuración de datos recibidos:</h3>";
        echo "<b>Entidad no especificada o no válida:</b> " . htmlspecialchars($modulo) . "<br><br>";
        echo "<b>Datos recibidos mediante REQUEST (GET / POST):</b><pre>";
        print_r($_REQUEST);
        echo "</pre>";
        exit();
}
