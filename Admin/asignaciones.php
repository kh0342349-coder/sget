...
// 1. CARGA DE IDIOMA IGUAL QUE EN ADMIN.PHP Y USUARIOS.PHP
...
// 3. Lógica para CREAR RESERVA Y ASIGNACIÓN
    ...
    try {
        if ($tipo_pasajero === 'ocasional') {
            ...
            if (empty($nombre_ocasional)) {
                throw new Exception($lang['err_nombre_ocasional'] ?? "El nombre del pasajero sin registro es obligatorio.");
            }
            ...
            if ($id_pasajero <= 0) {
                throw new Exception($lang['err_seleccionar_pasajero'] ?? "Debe seleccionar un pasajero válido.");
            }
        }
        ...
        if ($viaje) {
            ...
            if ($cupos_actuales >= $cantidad_puestos) {
                ...
                $msjExito = sprintf($lang['msj_asignacion_exito'] ?? "¡Asignación de %d puesto(s) registrada con éxito! Abriendo ticket...", $cantidad_puestos);
                $txtVerTicket = $lang['btn_ver_ticket'] ?? "Ver Ticket";
                ...
                <div class='bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 p-4 rounded-2xl shadow-lg flex items-center justify-between mb-6'>
                    <div class='flex items-center gap-3'>
                        ...
                        <span class='text-sm font-semibold'>{$msjExito}</span>
                    </div>
                    <a href='imprimir_ticket.php?id={$id_nueva_reserva}' target='_blank' class='bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold px-3 py-1.5 rounded-lg transition-all flex items-center gap-2'>
                        <i class='fas fa-file-pdf'></i> {$txtVerTicket}
                    </a>
                </div>" . $script_pdf;
            } else {
                $conexion->rollback();
                $msjSinCupos = sprintf($lang['msj_no_cupos'] ?? "No hay suficientes cupos disponibles. Solo quedan %d puestos.", $cupos_actuales);
                $mensaje = "
                <div class='bg-red-500/10 border border-red-500/20 text-red-400 p-4 rounded-2xl shadow-lg flex items-center gap-3 mb-6'>
                    ...
                    <span class='text-sm font-semibold'>{$msjSinCupos}</span>
                </div>";
            }
        }
    } catch (Exception $e) {
        $conexion->rollback();
        $txtErrInterno = $lang['err_interno'] ?? "Error interno";
        $mensaje = "
        <div class='bg-red-500/10 border border-red-500/20 text-red-400 p-4 rounded-2xl shadow-lg flex items-center gap-3 mb-6'>
            <i class='fas fa-bug text-lg'></i>
            <span class='text-sm font-semibold'>{$txtErrInterno}: " . htmlspecialchars($e->getMessage()) . "</span>
        </div>";
    }
}
...
$lblRutaPrefix = $lang['lbl_ruta_prefijo'] ?? 'Ruta #';
$sqlViajesDisponibles = "SELECT v.id_via, 
                                COALESCE(r.nom_rut, CONCAT('{$lblRutaPrefix}', v.id_rut_via)) as nom_rut, 
                                r.img_rut,
                                v.fec_via,
                                v.hor_sal_via, 
...
<head>
    ...
    <title>SGET - <?= $lang['tit_asignaciones'] ?? 'Asignaciones y Recaudo' ?></title>
...
<body class="bg-slate-50 dark:bg-[#080c14] text-slate-800 dark:text-slate-100 flex min-h-screen transition-colors duration-300">
                ...
                <div>
                    <div class="flex items-center gap-2.5">
                        <h1 class="text-2xl font-black text-slate-900 dark:text-white tracking-tight">
                            <?= $lang['tit_gestion_asig'] ?? 'Gestión de Asignaciones y Pagos' ?>
                        </h1>
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">
                        <?= ($idiomaActual === 'en') ? 'Select the trip card directly to add registered or walk-in passengers, verify payments, and reserve seats.' : 'Selecciona directamente el viaje en tarjeta para agregar pasajeros registrados o sin registro, comprobar pagos y apartar puestos.' ?>
                    </p>
                </div>
            </div>
            ...
            <div class="space-y-6">
                <div class="flex justify-between items-center bg-white dark:bg-[#121826] p-5 rounded-3xl border border-slate-200 dark:border-white/10 shadow-xl">
                    <h2 class="text-base font-extrabold text-slate-900 dark:text-white flex items-center gap-2">
                        <i class="fas fa-bus text-emerald-400"></i> <?= $lang['sub_viajes_activos'] ?? 'Viajes Activos y Cupos en Ruta' ?>
                    </h2>
                    <span class="text-[10px] font-mono text-slate-400 uppercase tracking-wider bg-black/20 px-3 py-1 rounded-xl border border-white/5">
                        <?= ($idiomaActual === 'en') ? 'Updated in real time' : 'Actualizado en tiempo real' ?>
                    </span>
                </div>
                ...
                <?php if($viajesDisponibles && $viajesDisponibles->num_rows > 0): ?>
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        ...
                        while($vd = $viajesDisponibles->fetch_assoc()): 
                            ...
                            $fechaSalida = !empty($vd['fec_via']) ? date('d/m/Y', strtotime($vd['fec_via'])) : (($idiomaActual === 'en') ? 'No date' : 'Sin fecha');
                            ...
                            <div class="relative overflow-hidden rounded-2xl h-64 border border-slate-200 dark:border-white/10 shadow-lg group transition-all duration-300 hover:shadow-2xl flex flex-col justify-between p-4 bg-slate-950">
                                ...
                                <div class="relative z-10 flex items-center justify-between mb-2">
                                    ...
                                    <?php if(intval($vd['cup_dis']) > 0): ?>
                                        <span class="text-[9px] font-extrabold uppercase tracking-wider text-emerald-300 bg-emerald-900/60 px-2.5 py-0.5 rounded-full border border-emerald-500/40 flex items-center gap-1 backdrop-blur-md">
                                            <span class="w-1.5 h-1.5 rounded-full bg-emerald-400 inline-block animate-pulse"></span> <?php echo $vd['cup_dis']; ?> <?= ($idiomaActual === 'en') ? 'free seats' : 'cupos libres' ?>
                                        </span>
                                    <?php else: ?>
                                        <span class="text-[9px] font-extrabold uppercase tracking-wider text-red-300 bg-red-900/60 px-2.5 py-0.5 rounded-full border border-red-500/40 backdrop-blur-md">
                                            <?= ($idiomaActual === 'en') ? 'Sold out' : 'Agotado' ?>
                                        </span>
                                    <?php endif; ?>
                                </div>
                                ...
                                <div class="relative z-10 space-y-0.5 my-auto">
                                    ...
                                    <div class="flex items-center justify-between text-[10px] text-slate-300 pt-1">
                                        <span><i class="fas fa-user-tie mr-1 text-slate-400"></i> <?php echo htmlspecialchars($vd['nom_conductor'] ?? (($idiomaActual === 'en') ? 'Unassigned' : 'Sin asignar')); ?></span>
                                ...
                                <div class="relative z-10 pt-3 border-t border-white/20">
                                    <?php if(intval($vd['cup_dis']) > 0): ?>
                                        <button type="button" 
                                            ...
                                            <i class="fas fa-user-plus text-xs"></i> <?= ($idiomaActual === 'en') ? 'Add Passenger' : 'Agregar Pasajero' ?>
                                        </button>
                                    <?php else: ?>
                                        <button type="button" disabled class="w-full py-2.5 bg-slate-800 text-slate-500 font-bold text-xs uppercase tracking-wider rounded-xl cursor-not-allowed">
                                            <?= ($idiomaActual === 'en') ? 'Sold Out' : 'Cupos Agotados' ?>
                                        </button>
                                    <?php endif; ?>
                                </div>
                ...
                <?php else: ?>
                    <div class="flex flex-col items-center justify-center p-12 bg-white dark:bg-[#121826] rounded-3xl border border-slate-200 dark:border-white/10 shadow-xl text-center">
                        ...
                        <h3 class="text-base font-bold text-slate-800 dark:text-white"><?= ($idiomaActual === 'en') ? 'No active trips' : 'No hay viajes activos' ?></h3>
                        <p class="text-slate-500 dark:text-slate-400 text-xs mt-1"><?= ($idiomaActual === 'en') ? 'There are currently no routes available for assignment.' : 'Actualmente no existen trayectos disponibles para asignación.' ?></p>
                    </div>
                <?php endif; ?>
            </div>
        ...
        <div class="bg-white dark:bg-[#121826] w-full max-w-md rounded-3xl p-6 border border-slate-200 dark:border-white/10 shadow-2xl space-y-4 transform scale-95 transition-all duration-300">
            ...
            <div class="flex justify-between items-center border-b border-slate-100 dark:border-white/5 pb-3">
                <div>
                    <h3 class="font-extrabold text-slate-900 dark:text-white text-base flex items-center gap-2">
                        <i class="fas fa-ticket-alt text-sky-400"></i> <?= ($idiomaActual === 'en') ? 'Passenger Assignment' : 'Asignación de Pasajero' ?>
                    </h3>
            ...
            <form action="asignaciones.php" method="POST" class="space-y-4">
                ...
                <div class="space-y-1.5">
                    <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider"><?= ($idiomaActual === 'en') ? 'Passenger Type' : 'Tipo de Pasajero' ?></label>
                    <div class="grid grid-cols-2 gap-2">
                        <button type="button" id="btnTipoRegistrado" onclick="cambiarTipoPasajero('registrado')" class="py-2 px-3 rounded-xl text-xs font-bold uppercase transition-all bg-sky-500 text-slate-950 shadow-sm cursor-pointer"><?= ($idiomaActual === 'en') ? 'Registered' : 'Registrado' ?></button>
                        <button type="button" id="btnTipoOcasional" onclick="cambiarTipoPasajero('ocasional')" class="py-2 px-3 rounded-xl text-xs font-bold uppercase transition-all bg-slate-100 dark:bg-white/5 text-slate-400 hover:text-white cursor-pointer"><?= ($idiomaActual === 'en') ? 'Walk-in (Occasional)' : 'Sin Registro (Ocasional)' ?></button>
                    </div>
                ...
                <div id="seccionRegistrado" class="space-y-1.5">
                    <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider"><?= ($idiomaActual === 'en') ? 'Select from List' : 'Seleccionar de la Lista' ?></label>
                    <select name="id_pasajero" id="select_id_pasajero" class="w-full px-4 py-3 bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-2xl outline-none focus:border-sky-400 text-xs text-slate-800 dark:text-white">
                        <option value=""><?= ($idiomaActual === 'en') ? 'Select registered passenger...' : 'Seleccione pasajero registrado...' ?></option>
                ...
                <div id="seccionOcasional" class="space-y-1.5 hidden">
                    <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider"><?= ($idiomaActual === 'en') ? 'Occasional Passenger Name' : 'Nombre del Pasajero Ocasional' ?></label>
                    <input type="text" name="nombre_ocasional" id="input_nombre_ocasional" placeholder="<?= ($idiomaActual === 'en') ? 'E.g., John Doe (Walk-in)' : 'Ej: Juan Pérez (Walk-in)' ?>" class="w-full px-4 py-3 bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-2xl outline-none focus:border-sky-400 text-xs text-slate-800 dark:text-white">
                </div>

                <div class="grid grid-cols-2 gap-3">
                    <div class="space-y-1.5">
                        <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider"><?= ($idiomaActual === 'en') ? 'Seats Quantity' : 'Cantidad de Puestos' ?></label>
                    ...
                    <div class="space-y-1.5">
                        <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider"><?= ($idiomaActual === 'en') ? 'Total Value' : 'Valor Total' ?> ($)</label>
                ...
                <div class="space-y-1.5">
                    <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider"><?= ($idiomaActual === 'en') ? 'Payment Confirmation / Method' : 'Confirmación / Método de Pago' ?></label>
                    <select name="metodo_pago" required class="w-full px-4 py-3 bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-2xl outline-none focus:border-sky-400 text-xs text-slate-800 dark:text-white">
                        <option value="Efectivo"><?= ($idiomaActual === 'en') ? 'Cash (Confirmed at counter)' : 'Efectivo (Confirmado en ventanilla)' ?></option>
                        <option value="Transferencia"><?= ($idiomaActual === 'en') ? 'Bank Transfer (Verified)' : 'Transferencia Bancaria (Comprobado)' ?></option>
                        <option value="Tarjeta"><?= ($idiomaActual === 'en') ? 'Card / POS (Approved)' : 'Tarjeta / POS (Aprobado)' ?></option>
                    </select>
                </div>

                <div class="pt-2 flex gap-3">
                    <button type="button" onclick="cerrarModalAsignar()" class="flex-1 py-3 bg-slate-100 dark:bg-white/5 text-slate-300 rounded-xl text-xs font-bold uppercase tracking-wider cursor-pointer"><?= $lang['cfg_cancelar'] ?? 'Cancelar' ?></button>
                    <button type="submit" name="asignar" class="flex-1 py-3 bg-gradient-to-r from-sky-500 to-blue-600 text-white font-extrabold rounded-xl text-xs uppercase tracking-wider shadow-lg shadow-sky-500/20 hover:opacity-90 transition-all cursor-pointer">
                        <?= ($idiomaActual === 'en') ? 'Confirm & Generate' : 'Confirmar y Generar' ?>
                    </button>
                </div>
            </form>
    ...
    <div id="modalAyuda" class="fixed inset-0 z-50 flex items-center justify-center pointer-events-none opacity-0 transition-all duration-300 p-4">
        <div class="bg-white dark:bg-[#121826] w-full max-w-md rounded-3xl p-6 border border-slate-200 dark:border-white/10 shadow-2xl space-y-4 transform scale-95 transition-all duration-300">
            <div class="flex justify-between items-center border-b border-slate-100 dark:border-white/5 pb-3">
                <h3 class="font-extrabold text-slate-900 dark:text-white text-base flex items-center gap-2">
                    <i class="fas fa-info-circle text-sky-400"></i> <?= ($idiomaActual === 'en') ? 'Assignments & Payments Guide' : 'Guía de Asignaciones y Pagos' ?>
                </h3>
            ...
            <ul class="space-y-2.5 text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
                <li class="flex items-start gap-2">
                    ...
                    <span><b><?= ($idiomaActual === 'en') ? 'Registered or Walk-in Passengers:' : 'Pasajeros Registrados u Ocasionales:' ?></b> <?= ($idiomaActual === 'en') ? 'Choose between selecting a user from the list or entering the name of an unregistered passenger.' : 'Puedes elegir entre seleccionar un usuario de la lista o ingresar el nombre de un pasajero sin registro.' ?></span>
                </li>
                <li class="flex items-start gap-2">
                    <i class="fas fa-chair text-emerald-400 mt-0.5"></i>
                    <span><b><?= ($idiomaActual === 'en') ? 'Seats and Payment:' : 'Puestos y Pago:' ?></b> <?= ($idiomaActual === 'en') ? 'Specify the required number of seats and the system will automatically calculate the total price.' : 'Indica la cantidad de asientos requeridos y el sistema calculará automáticamente el valor total.' ?></span>
                </li>
                <li class="flex items-start gap-2">
                    <i class="fas fa-file-pdf text-purple-400 mt-0.5"></i>
                    <span><b><?= ($idiomaActual === 'en') ? 'Automatic Ticket:' : 'Ticket Automático:' ?></b> <?= ($idiomaActual === 'en') ? 'After confirming the transaction, seats will be deducted and the PDF voucher will open.' : 'Tras confirmar la transacción, se descontarán los cupos y se abrirá el PDF con el comprobante.' ?></span>
                </li>
            </ul>
    ...
    function abrirModalAsignar(idViaje, nombreRuta, cuposDisponibles, precioRuta) {
        ...
        const txtRuta = IDIOMA_ACTUAL === 'en' ? 'Route' : 'Ruta';
        const txtDisponibles = IDIOMA_ACTUAL === 'en' ? 'Available' : 'Disponibles';
        
        document.getElementById('modalRutaTitulo').innerText = txtRuta + ': ' + nombreRuta + ' (' + txtDisponibles + ': ' + cuposDisponibles + ')';
